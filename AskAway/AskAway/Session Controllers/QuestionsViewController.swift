...
class QuestionsViewController: UIViewController, IndicatorInfoProvider, UITableViewDelegate, UITableViewDataSource {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let index = questionTable.indexPathForSelectedRow{
            let chatVC = segue.destination as! ChatViewController
            chatVC.currentSession = session!
            if index.section == 0 {
                chatVC.currentQuestion = self.DB.answered[index.row]
            } else {
                chatVC.currentQuestion = self.DB.unanswered[index.row]
    ...
    @objc func handleLongPress(recognizer: UILongPressGestureRecognizer){
        if recognizer.state == UIGestureRecognizerState.began {
            ...
            if let indexPath = questionTable.indexPathForRow(at: touchPoint) {
                ...
                let tagButton = UIAlertAction(title: "Tag this question as answered",style: .default, handler: {(action) -> Void in
                    self.DB.tagQuestionAnswered(session: self.session!.firebaseID, question: question!.questionID, callback: {
                        self.refresh()
                    })
                })
                ...
                if indexPath.section == 0{
                    ...
                    alertController!.addAction(deleteButton)
                    alertController!.addAction(tagButton)
                    alertController!.addAction(cancelButton)
                }
